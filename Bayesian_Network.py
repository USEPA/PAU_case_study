...
def drawing_network(df, dir_path):
    ...
    for idx, row in df.iterrows():
        if row['Target'] == 'Node-17':
            color = '#1b6ca8'
        elif row['Source'] == 'Node-18':
            color = '#ff847c'
        elif row['Target'] == 'Node-19':
            color = '#e84a5f'
        else:
            color = 'black'
...
def building_dataframe(dir_path, CAS, Years, values):
    ...
    df = df.loc[df['CAS NUMBER'] == CAS]
    df.drop(columns = ['TRIFID', 'CAS NUMBER'], inplace = True)
    ...
    cols_for_change.update({col: col.capitalize() for col in set(cols_for_using) - set(cols_for_change.keys())})
    ...
    Levels_for_PCU = list(df['PCU'].unique())
    return (df , Levels_for_PCU)
...
def Building_flows_dataset(dir_path, Years, nbins):
    df =  pd.DataFrame()
    for Year in Years:
        df_year = pd.read_csv(dir_path + '/Waste_flow/Waste_flow_to_PCUs_{}_10.csv'.format(Year),
                     usecols = ['METHOD CODE', 'MIDDLE WASTE FLOW', 'TRIFID',
                                'TYPE OF MANAGEMENT', 'WASTE STREAM CODE',
                                'PRIMARY NAICS CODE'],
                     low_memory = False)
    ...
    df_intervals.to_csv(dir_path + '/Relationship_flow_interval_and_codes.csv',
                        sep = ',', index = False)
    del df_intervals
    df = df[['METHOD CODE', 'MIDDLE WASTE FLOW INTERVAL CODE',
            'WASTE STREAM CODE', 'TYPE OF MANAGEMENT']]
    df.drop_duplicates(keep = 'first', inplace = True)
    df.rename(columns = {'METHOD CODE': 'PCU',
                         'MIDDLE WASTE FLOW INTERVAL CODE': 'Waste flow',
...
def Building_price_dataset(dir_path, Years, nbins):
    df =  pd.DataFrame()
    for Year in Years:
    ...
    df_intervals.to_csv(dir_path + '/Relationship_chemical_prices_and_codes.csv',
                        sep = ',', index = False)
    del df_intervals
    df = df[['METHOD CODE - 2004 AND PRIOR',
            'PRICE INTERVAL CODE']]
    df.drop_duplicates(keep = 'first', inplace = True)
    df.rename(columns = {'METHOD CODE - 2004 AND PRIOR': 'PCU',
                         'PRICE INTERVAL CODE': 'Chemical price'},
              inplace = True)
    return df


def Building_PAOC_and_PACE_dataset(dir_path, nbins):
    ...
    df_PAOC.rename(columns = {'Activity': 'Type of waste management',
                              'Media': 'Type of waste',
                              'MEAN PAOC INTERVAL CODE': 'PAOC'},
                    inplace = True)
    df_PAOC =  df_PAOC[['Type of waste management', 'Type of waste', 'PAOC', 'NAICS code']]
    ...
    df_PACE.rename(columns = {'Activity': 'Type of waste management',
                              'Media': 'Type of waste',
                              'MEAN PACE INTERVAL': 'PACE'},
                    inplace = True)
    df_PACE = df_PACE[['NAICS code', 'PACE', 'Type of waste management', 'Type of waste']]
    return (df_PAOC, df_PACE)
...
if __name__ == '__main__':
    ...
    if CAS_for_search:
        ...
        try:
            # Waste flows
            df_flows = Building_flows_dataset(dir_path, Years, N_Bins)
            # Chemical prices
            df_chemical = Building_price_dataset(dir_path, Years, N_Bins)
            df_merged = pd.merge(df_chemical, df_flows, on = 'PCU', how = 'right')
            del df_flows, df_chemical
            # Pollution abatement operating cost (PAOC)
            # Pollution abatement capital expenditure(PACE)
            df_PAOC, df_PACE = Building_PAOC_and_PACE_dataset(dir_path, N_Bins)
            print(df_PAOC.info())
            print(df_PACE.info())
        except FileNotFoundError:
            pass
        for chem in CAS_for_search:
            df, Levels_for_PCU = building_dataframe(dir_path, chem, Years, values)
            # Inputs_list.update({'Chemical': Levels_for_chemical, 'PCU': Levels_for_PCU})
            # Probabilities = buidling_probabilities(df, edges, Inputs_list)
