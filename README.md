...
1. In the folder [marginal](https://github.com/jodhernandezbe/PAU_case_study/tree/master/bayesian_network/probabilities/marginal), the files, whose name have the structure Marginal_probabilities_based_on_BN_for_**CAS**_in_stream_**#**.csv, have the marginal proabilities under the BN. **CAS** indicates the chemical, while **#** the number for the stream that contains the chemical.
