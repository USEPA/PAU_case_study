...
def method_of_moments(Flow_vals):
...
def estimating_val_with_log(mu, theta_2):
    try:
        Value = lognorm.rvs(s=theta_2**0.5,
                           scale=np.exp(mu))
    except ValueError:
        try:
            Value = lognorm.rvs(s=10**-9,
                               scale=np.exp(mu))
        except ValueError:
            Value = 0.0
    return Value
...
def checking_outliers(vals):
    scores = stats.zscore(vals).round(0)
    scores = np.abs(scores)
    vals = pd.Series([vals[i] for i in range(len(vals)) if scores[i] < 3])
    return vals


def non_zero_output_streams(PAU_flows):
    PAU_flows = PAU_flows.loc[~PAU_flows['Type of stream'].isin(['Effluent',
                              'Remanent'])]
    PAU_flows = PAU_flows[['Type of stream', 'Mean quantity [kg/yr]']]\
        .groupby('Type of stream', as_index=False).sum()
    List_of_non_zero = list()
    for idx, row in PAU_flows.iterrows():
